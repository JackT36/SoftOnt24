...
*   Local vars                                                                *
******************************************************************************/
// Test bitmap (16x16 pixels, monochrome)
typedef struct {
    int bitmap[BITMAP_HEIGHT][BITMAP_WIDTH];
} Bitmap;

// Example bitmaps with color information
Bitmap bitmaps[] = {
    {
        .bitmap = {
            {0xE1, 0xE1, 0xE1, 0xE1, 0xE1, 0xE1, 0xE1, 0xE1, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF},
            {0xE1, 0xE1, 0xE1, 0xE1, 0xE1, 0xE1, 0xE1, 0xE1, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF},
            {0xE1, 0xE1, 0xE1, 0xE1, 0xE1, 0xE1, 0xE1, 0xE1, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF},
            {0xE1, 0xE1, 0xE1, 0xE1, 0xE1, 0xE1, 0xE1, 0xE1, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF},
            {0xE1, 0xE1, 0xE1, 0xE1, 0xE1, 0xE1, 0xE1, 0xE1, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF},
...
//int* bitmaps[] = {bitmap1, bitmap2};
/******************************************************************************
*   Local function prototypes                                                 *
******************************************************************************/
int DrawLine(int x1, int x2, int y1, int y2, int color);
void DrawBitmap(int x, int y, int bitmap[BITMAP_HEIGHT][BITMAP_WIDTH], int width, int height);
...
int API_draw_bitmap(int x_lup, int y_lup, int bm_nr)
{
    ...
    if (x_lup < 0 || y_lup < 0 || x_lup >= VGA_DISPLAY_X || y_lup >= VGA_DISPLAY_Y)
    {
        LOGE("Start position out of bounds");
        return -1;
    }

    Bitmap bitmap = bitmaps[bm_nr];
    //int* bitmap = bitmaps[bm_nr];
    int width = BITMAP_WIDTH;   // Width of the bitmap
    int height = BITMAP_HEIGHT; // Height of the bitmap
    //int color = 1;          // Color to use for the bitmap

    DrawBitmap(x_lup, y_lup, bitmap.bitmap, width, height);
...
void DrawBitmap(int x, int y, int bitmap[BITMAP_HEIGHT][BITMAP_WIDTH], int width, int height)
{
    for (int j = 0; j < height; j++)
    {
        ...
        {
            int color = bitmap[j][i];
            //if (color != ) //option for not drawing over background need to specify a "transparent" value
            //{
